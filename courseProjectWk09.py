    ...
    if strUserInput.lower() in ['y', 'yes', 'yep', 'yup', 'yeah']:
        ...
        with open("courseProjectWk06Anlysis.txt", "w") as wrapperLogFile:
            ...
            for strLogLine in listLogLines:
                listReturn = parseLogEntry(strLogLine)
                strIP = listReturn[0]
                strReturnCode = listReturn[1]
                if strReturnCode >= "400":
                    print(f"{strIP} - {strReturnCode}")
            ...
            with open("courseProjectWk09Analysis.csv", "w") as wrapperLogFile:
